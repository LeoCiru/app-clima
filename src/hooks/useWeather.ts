...
function useWeather() {
  ...
  const [loading, setLoading] = useState(false);
    ...
    async function fetchWeather(search: SearchType) {
        ...
        const urlGeo = `https://api.openweathermap.org/geo/1.0/direct?q=${search.city ? search.city.replace(/ /g, "%").toLowerCase() : search.city.toLowerCase},${search.country}&appid=${appID}`;
